# Converted to Python 3.1 by Jeremy Dreese, 2011-11-17.
...
retrieving the name of the Web site pointed to by the URL."""
    ...
    def isCrawlable(self):
        ...
        # accept hosts inside Bucknell and on gutenberg.org.
        #
        try:
        ...
        if len(ip) < 8:
            return False
        if not ip[0:7] == '134.82.'  and  not ip == '152.19.134.47':
            return False
        return self.isSimpleURL()

    class __neverFetch:
        """ Class to substitute for a robot when fetching robots.txt fails.
            In this case, the class always informs users that files cannot be
            fetched.
        """
        def __init__(self):
            pass
    ...
    def isForbidden(self):
        ...
        return not self.__robotDictionary[self.__site].can_fetch('*', str(self))
...
class WebPage(html.parser.HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        """ Process an anchor (<A>) tag.  The tag is converted to
            ...
            so, it is saved in the list of unique links; otherwise it is
            silently discarded.
        ...
        if tag == 'a':
            for attr, value in attrs:
                if attr == 'href':
                    if value == '':
                        return
                    value = self.__canonicalizeURL(value)
                    if value != '':
                        self.__anchorlist += [value]
    ...
    def __canonicalizeURL(
        ...
        if location != ''  or  path == '':
            ...
            #
            # The URL is relative.  Concatenate it with the base URL
            # and return it.  In this case it is always local.
            #
            if location == '':
            ...
            if path[0] == '/':
                ...
                if baseDir == '':
                    ...
                    # last component.
                    #
                    lastSlash = str.rfind(baseDir, '/')
                    if lastSlash > 0:
                        #
                        # Strip the last component from the path,
                        # leaving the slash.
                        #
                        baseDir = baseDir[0:str.rindex(baseDir, '/') + 1]
                    else:
                        #
                ...
                return urllib.parse.urlunparse((scheme, location,
                  WebPage.__normPath(baseDir + path),
                    parameters, query, fragment))

    __uselessDot = re.compile(r'/\./(\./)*')
    __multiSlash = re.compile(r'//+/')
    __upReference = re.compile(r'(/[^/]+/\.\./)')

    @staticmethod
    def __normPath(path):
        """ Normalize a path by removing redundant separators and up-level
            references.  This is like os.path.normpath except that it doesn't
            introduce backslashes on Windows.
        """
        if path.startswith('./'):
            path = path[2:]
        path = WebPage.__uselessDot.sub('/', path)
        path = WebPage.__multiSlash.sub('/', path)
        oldPath = path + 'X'
        while len(oldPath) != len(path):
            oldPath = path
            path = WebPage.__upReference.sub('/', path)
        return path
...
def getHTML(url):
    ...
    return (page.text().lower(), page.urls())
